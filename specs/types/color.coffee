...
describe "Color", ->
  ...
  describe 'W3C Test Cases', ->
    it 'hsl and rgb should produce same', ->
      for colors in TEST_DATA_COLOR.split("\n")
        [hsl,rgb] = colors.split ":"
        expect(new Color(hsl).hex).toBe new Color(rgb).hex

  describe 'constructor', ->
    it 'should create color from different formats', ->
      x = ->
  ...
  describe 'hex', ->
    ...
    it "should return hex presentation", ->
      c = new Color
      expect(c.hex).toBe "FFFFFF"
  ...
  describe 'alpha', ->
    it 'should set value', ->
      c = new Color()
      c.alpha = 80
      expect(c.rgba).toBe "rgba(255, 255, 255, 0.8)"
  ...
  describe 'red', ->
    it 'should set value', ->
      c = new Color()
      c.red = 80
      expect(c.rgb).toBe "rgb(80, 255, 255)"
    it 'should return red value', ->
      c = new Color()
      expect(c.red).toBe 255
      c.red = 50
      expect(c.red).toBe 50

  describe 'green', ->
    it 'should set value', ->
      c = new Color()
      c.green = 80
      expect(c.rgb).toBe "rgb(255, 80, 255)"
  ...
  describe 'blue', ->
    it 'should set value', ->
      c = new Color()
      c.blue = 80
      expect(c.rgb).toBe "rgb(255, 255, 80)"
  ...
  describe 'lightness', ->
    it 'should set lightness', ->
      ...
      c.lightness = 0
      expect(c.hex).toBe "000000"
    it 'should set lightness is saturation is 0', ->
      c = new Color("#000000")
      c.lightness = 10
      expect(c.hex).toBe "1A1A1A"
    it 'should return lightness value', ->
      c = new Color()
      expect(c.lightness).toBe 100
      c.lightness = 80
      expect(c.lightness).toBe 80
  ...
  describe 'saturation', ->
    it 'should set saturation', ->
      ...
      expect(c.hex).toBe "FF0000"
      c.saturation = 80
      expect(c.hex).toBe "E61919"
    it 'should return saturation value', ->
      c = new Color()
      expect(c.saturation).toBe 0
      c.saturation = 80
  ...
  describe 'invert', ->
    it 'should invert colors', ->
      expect(new Color("#112233").invert().hex).toBe "EEDDCC"
      expect(new Color("rgba(245, 235, 225, 0.5)").invert().rgba).toBe "rgba(10, 20, 30, 0.5)"

  for colors in ['rgb:rgb(255, 255, 255)','rgba:rgba(255, 255, 255, 1)','hsl:hsl(0, 0%, 100%)','hsla:hsla(0, 0%, 100%, 1)']
    [type,result] = colors.split ":"
    c = new Color()
    describe type, ->
      it "should return #{type} representation", ->
        expect(c[type]).toBe result